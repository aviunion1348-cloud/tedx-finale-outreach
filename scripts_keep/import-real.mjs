...
const speakers = data.map((r, i) => {
  ...
  const followersRaw = (col(r, "followers") || "").trim();
  const verifiedFollowers = followersRaw ? parseInt(followersRaw) || 0 : 0;
  const branch = (col(r, "branch") || "INTERDISCIPLINARY").toUpperCase();
  const validBranches = ["CSE", "CSE_AI_ML", "ECE", "EEE", "BCA", "BSC_AI_DS", "BSC_BA", "BSC_DESIGN", "BBA", "BHM", "INTERDISCIPLINARY"];
  const branchTag = validBranches.includes(branch) ? branch : "INTERDISCIPLINARY";
  ...
  return {
    ...
    audience: {
      followers: verifiedFollowers || 0,
      mediaMentions: 12 + (i % 25),
      estimatedDrawStudents: 0,
